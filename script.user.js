...
(function() {
   ...
   // $(document.body).append(`<script src="https://images.haodata.xyz/notiflix-aio-2.7.0.min.js"></script>`)
  //  $(document.body).append(`<script src="https://images.haodata.xyz/pickmeup.min.js"></script>`)
    ...
    function getTable(ld){
        ...
        GM_xmlhttpRequest({
            url:getTableUrl+'?brandId=0&planId=0&originalMaterialId=0&launchId='+ld.launchId+'&beginDate='+getYesterday()+'&endDate='+getYesterday()+'&tabIds=T1001%2CT1002%2CT1003%2CT1004%2CT1005%2CT1010&groupByDimension=1',
            method :"get",
            onload:function(xhr){
                var data=$.parseJSON( xhr.responseText )
                //解析ncpm数据放入map
                ...
                if(launchesCount<launches.length){
                    ...
                    succ('结束爬取-NCPM数据：花费、曝光、点击、点击均价')
                    console.log('NCPM数据：花费、曝光、点击、点击均价',ncpmMap)
                    //智选展位跑完，可以跑其他的了
                    succ('开始爬取-门店统计：入口图点击率')
        ...
        $.when(
            ...
            GM_xmlhttpRequest({
                ...
                onload:function(xhr){
                    ...
                    cloaseLoading()
                    Notiflix.Confirm.Show( '提示',poiInfoss.length+'条店铺数据已经成功上传,是否登入好数据分析平台标记是否生成日报?', '确认', '暂不',function(){window.open('https://wanhao.haodata.xyz',"_blank","")}, function(){  } )
                }
            })
        })
    }
            ...
            document.querySelector('#pickmeupdata').addEventListener('pickmeup-change', function (e) {
                ...
                if(clickNum%2==0){
                    ...
                    if(accountInfo.account!=null){
                        window.location.href=mainUrl+'ecome/export/week/'+accountInfo.account+'/'+st+'/'+et
                    }else{
                        $.get(getAccountUrl+'?_tm='+getNowTime(),function(data){
                            var account=data.user.userId
